...
**Последнее обновление**: 2026-07-01 (этап 8 закрыт: правило «не выдумывай» проверено в боте)
**Текущий этап**: Этап 9 — Загрузка пользовательского фото в S3 (☐ не начат)
**Следующий шаг**: helper/навык загрузки — человек кидает фото в Telegram (или даёт URL) → агент льёт в Beget S3 (доступы в `s3.txt`, Signature v4, `boto3`/`aws`/подписанный PUT) → отдаёт публичную ссылку. Проверить схему публичной ссылки (path-style `{endpoint}/{bucket}/{key}` vs virtual-hosted). Интеграция: загруженное фото → в `images` черновика как `role:raw`. Затем деплой (этап 10) — последним.
...
| 9 | Загрузка пользовательского фото в S3 | 🚧 | `stage-9-done` | — | — |
