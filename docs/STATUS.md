...
**Последнее обновление**: 2026-07-01 (этап 9 закрыт; исправлен баг проброса секретов в sandbox)
**Текущий этап**: Этап 10 — Деплой на VPS (☐ не начат)
**Следующий шаг**: деплой агента + Postgres на VPS `89.125.17.86` (Docker) через `/okdeploy`. В серверный `.env` перенести `TAVILY_API_KEY` + `S3_*`; в серверный `config.yaml` — `web.backend: tavily` и `terminal.env_passthrough` (KIE/CONTENT/POSTMYPOST/S3); перенести `SOUL.md` и cron-обвязку. Блокер: прод-ключ OpenRouter клиента.
...
| 10 | Деплой на VPS | ☐ | `stage-10-done` | — | — |

Детальные критерии приёмки каждого этапа — в [`07_ROADMAP.md`](07_ROADMAP.md).

## Активная работа

**2026-07-01 — добавлены этапы 7–9 по запросу клиента, деплой перенумерован 7→10.** Этапы 7 (веб-поиск), 8 (запрет выдумок), 9 (загрузка фото в S3) закрыты в тот же день. Плюс исправлен баг проброса секретов навыков в sandbox `execute_code` (см. блок в шапке + «на деплой»). **Следующий и последний — этап 10 (деплой на VPS).**
