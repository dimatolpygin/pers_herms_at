...
**Последнее обновление**: 2026-07-01 (этап 9 закрыт: загрузка фото в S3 проверена в боте)
**Текущий этап**: между этапами — разбираем баг проброса секретов в sandbox (`execute_code`), затем этап 10 (деплой)
**Следующий шаг**: РАЗОБРАТЬ БАГ — агент часто пишет «ключ KIE не пробросился в sandbox» и обходит через `terminal`. Секреты (KIE_AI_API_KEY и др.) не доходят до песочницы `execute_code`, хотя терминал их видит. На этапе 4 это лечили `required_environment_variables` во фронтматтере content-studio — проверить, что там KIE_AI_API_KEY объявлен и копия навыка синхронна. Затем этап 10 (деплой на VPS), в серверный `.env` перенести `TAVILY_API_KEY` + `S3_*`.
...
**2026-07-01 — добавлены этапы 7–9 по запросу клиента, деплой перенумерован 7→10.** Этапы 7 (веб-поиск) и 8 (запрет выдумок) закрыты в тот же день. **Следующий — этап 9 (загрузка фото в S3).** Деплой — этап 10, последним.
