...
**Последнее обновление**: 2026-07-01 (добавлены этапы 7–9; деплой перенумерован 7→10)
**Текущий этап**: Этап 7 — Веб-поиск: включить рабочий бэкенд (☐ не начат)
**Следующий шаг**: включить веб-поиск hermes. Диагностика (2026-07-01): поиск выключен полностью — в `.env` все ключи закомментированы, секции `web:` в `config.yaml` нет, DDGS не в авто-детекте. Нужно выбрать бэкенд (с клиентом: Tavily/Exa free 1000/мес с extract — рекомендую, либо DDGS без ключа как fallback), прописать `web.backend` + ключ, проверить `web_search` из бота. Затем этап 8 (правило «не выдумывай») и этап 9 (S3-загрузка фото), деплой — последним.
...
| 7 | Веб-поиск: включить рабочий бэкенд | 🚧 | `stage-7-done` | — | — |
...
**2026-07-01 — добавлены этапы 7–9 по запросу клиента, деплой перенумерован 7→10.** Новое: этап 7 (включить веб-поиск), этап 8 (запрет выдумок), этап 9 (загрузка фото пользователя в S3). Деплой теперь этап 10 — последним.

**Диагностика веб-поиска (2026-07-01):** поиск у агента выключен полностью — корень и «нет поиска», и «выдумывает». Факты: в `%LOCALAPPDATA%\hermes\.env` все ключи (Firecrawl/Tavily/Exa/Parallel/Brave) закомментированы, `SEARXNG_URL` пуст; в `config.yaml` секции `web:` нет; DDGS (единственный free без ключа) не входит в цепочку авто-детекта → сам не включается. Значит `web_search`/`web_extract` недоступны. Из коробки hermes даёт эти инструменты — писать своё не надо, надо выбрать и включить бэкенд (детали и кандидаты — в этапе 7 ROADMAP). Живой прогон через бота — при взятии этапа в работу (в sandbox venv/python hermes не виден). S3-доступы для этапа 9 — в `s3.txt` (Beget, S3-совместимый, Signature v4).

Этап 6 закрыт (тег `stage-6-done`, 2026-07-01). Автопостинг по крону работает вживую: `cron/autopost.py` (stdlib, без LLM) + launcher `cron/run-autopost.bat` + миграция `0003` (статус `publishing`). Локально гоняли через Windows Task Scheduler (каждую 1 мин); **после теста задачу удалили** (`Unregister-ScheduledTask HermesAutopost`) — на проде будет Linux cron.
