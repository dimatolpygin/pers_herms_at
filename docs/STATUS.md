...
**Последнее обновление**: 2026-07-01 (этап 7 закрыт: веб-поиск Tavily работает в боте)
**Текущий этап**: Этап 8 — Запрет выдумок (anti-hallucination) (☐ не начат)
**Следующий шаг**: добавить правило в SOUL.md — на свежие/проверяемые вопросы сначала `web_search` (уже рабочий, Tavily), ответ с источником; если поиск пуст/недоступен — честное «не знаю», без выдумок. Отделить творческие задачи (тексты постов/КП). Прогнать контрольные вопросы-ловушки. Затем этап 9 (S3-загрузка фото), деплой — последним.
...
| 8 | Запрет выдумок (anti-hallucination) | 🚧 | `stage-8-done` | — | — |
...
**2026-07-01 — добавлены этапы 7–9 по запросу клиента, деплой перенумерован 7→10.** Этап 7 (веб-поиск) закрыт в тот же день. **Следующий — этап 8 (запрет выдумок).** Этап 9 — загрузка фото пользователя в S3 (доступы в `s3.txt`, Beget, S3-совместимый, Signature v4). Деплой — этап 10, последним.

**Веб-поиск (этап 7, закрыт):** Tavily основной (`config.yaml` → `web.backend: tavily`, ключ в `.env`), Exa+Firecrawl резерв. Работает вживую в боте. Детали — в «Истории закрытий».

**Этап 8 (план):** правило в SOUL.md «не знаешь → `web_search`, ответ с источником; пусто/недоступно → честное "не знаю", без выдумок». Отделить творческие задачи (тексты постов/КП, там выдумка уместна) от фактических. Прогнать вопросы-ловушки. Опирается на уже рабочий поиск этапа 7. Автопостинг по крону работает вживую: `cron/autopost.py` (stdlib, без LLM) + launcher `cron/run-autopost.bat` + миграция `0003` (статус `publishing`). Локально гоняли через Windows Task Scheduler (каждую 1 мин); **после теста задачу удалили** (`Unregister-ScheduledTask HermesAutopost`) — на проде будет Linux cron.
