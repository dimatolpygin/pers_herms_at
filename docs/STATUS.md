...
**Последнее обновление**: 2026-07-01 (добавлены этапы 10–11; деплой перенумерован 10→12)
**Текущий этап**: Этап 10 — cron-задачи (напоминания/регулярки по запросу) (☐ не начат)
**Следующий шаг**: тонкий навык-гид над нативным инструментом `cronjob` (hermes уже умеет крон-задачи по запросу) — русские триггеры «напомни…/каждый день…», время МСК, самодостаточный prompt, показ/удаление задач. Референс: `D:\claude\stady\.pi\skills\cron-tasks`. НЕ трогать `cron/autopost.py` (этап 6). Затем этап 11 (правка своих навыков — только edit существующих, нативный `skill_manager` + штатный write-gate), деплой — этап 12, последним. Блокер деплоя: прод-ключ OpenRouter клиента.
...
| 10 | cron-задачи (напоминания/регулярки по запросу) | ☐ | `stage-10-done` | — | — |
...
**2026-07-01 — добавлены этапы 10–11, деплой перенумерован 10→12.** По запросу клиента два новых навыка, оба **нативные в hermes** (движок не пишем): (10) cron-задачи по запросу — напоминания/регулярки через встроенный `cronjob` (не путать с `autopost.py` этапа 6); (11) правка своих навыков — **только edit существующих** через встроенный `skill_manager` + штатный write-gate (создание новых с нуля в объём не входит, решение клиента). Обоим нужен тонкий навык-гид (русские триггеры) + проверка, что инструмент включён в Telegram-тулсете. **Следующий — этап 10.** Деплой — этап 12, последним.
