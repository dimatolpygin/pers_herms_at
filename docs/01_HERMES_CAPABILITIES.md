...
1. **kie.ai (gpt-image-2) как провайдер картинок** — в `plugins/image_gen/` нет kie.ai. Для этапа 4 добавлена skill-обёртка `content-studio` (createTask → polling recordInfo → resultUrls/download); ждёт живой Telegram/kie.ai UAT.
2. **Навык docx (КП) + csv (отчёты)** — генерация деловых документов и отправка файлом. Этап 3.
3. **Навык «контент»** — гибкий сценарий: вход свободный/ссылка/Tilda → 3 текста + картинка → превью. Исходник `hermes-skills/marketing/content-studio`, установлен в `%LOCALAPPDATA%\hermes\skills\marketing\content-studio`; stage 4 ещё открыт до живой проверки.
