...
MACHINE_BLOCKS = [
    ...
    r"\[Image attached.*?\]",
    r"\[Replying to:\s*\".*?\"\]",
    # Уведомление диспетчера kanban о закрытой карточке приходит как role='user'.
    # Без этой строки замер «после» ломается зеркально: успешное делегирование
    # порождает врезку «[kanban] Task … completed», она открывает НОВУЮ задачу,
    # и та считается «Гера сделал сам». Чем лучше делегирует — тем хуже цифра.
    r"\[kanban\].*?(?=\Z)",
]
