...
def refusal(name, zone):
    # Текст обязан быть НЕйтральным к профилю: раньше здесь стояло «свои
    # образцы, реквизиты и критик, который пересчитывает суммы» — верно для
    # docs и бессмысленно для dev, которому в отказе рассказывали про суммы.
    return (
        u"Это зона профиля `%s` (%s). Делать самому нельзя: у профиля своя "
        u"модель, свои навыки и свой критик из другой семьи моделей — у тебя "
        u"этого нет, и результат выйдет хуже.\n\n"
